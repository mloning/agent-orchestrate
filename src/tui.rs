...
fn render_table(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let header = Row::new(["STATE", "AGE", "SESSION", "TOPIC"])
    ...
    let state_w = app
        .agents
        .iter()
        .map(|a| a.status.label().len())
        .max()
        .unwrap_or(0)
        .max("STATE".len()) as u16;

    let widths = [
        Constraint::Length(state_w), // STATE — widest label currently shown
        Constraint::Length(7),       // AGE ("<1 min", "59 min")
        Constraint::Length(25),      // SESSION — tmux session name; the daily switch anchor
        Constraint::Fill(1),         // TOPIC — stable session label, fills the remainder
    ];
